...
class Engine:
    ...
    def create_index(self):
        schema = Schema(title=TEXT(stored=True), path=ID(stored=True), content=TEXT(stored=True), embedding=TEXT(stored=True))
    ...
    def index_doc(self, title, content, path):
        ...
        for chunk in chunks:
            chunk_embedding = self.model.encode(chunk).tolist()
            writer.add_document(title=title, content=chunk.strip(), path=path, embedding=str(chunk_embedding))
        writer.commit()
